...
class _StatsHomeState extends State<StatsHome> {
  ...
  void initState() {
    ...
    weakHskList = getStats(
      ...
      orderBy: "ASC",
      deckSize: 10,
      where: "WHERE wrong_occurrence > 0",
    );
    recentHskList = getStats(
      sortBy: "last_seen",
      orderBy: "DESC",
      deckSize: 10,
    );
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                ...
                BuildTotalStats(globalStats: globalStats),
                const SizedBox(height: 10),
                _HorizontalHskList(
                  hskList: recentHskList,
                  title: "Recent Words",
...
class StatsOverView extends StatelessWidget {
  ...
  final Future<List<Map<String, dynamic>>> timelineList;
          ...
          FutureBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (
              ...
              if (snapshot.hasData) {
                ...
                return const _StatsChart(
                  timeline: [
                    {
                      "string_date": "2022-10-26",
                      "right_occurrence": 0,
                      "wrong_occurrence": 0,
                      "new_word": 0,
                      "total": 0,
            ...
            builder: (
              ...
              if (snapshot.hasData) {
                ...
                return _QuickStats(
                  ...
                  reviewWords: reviewWords.toString(),
...
class BuildTotalStats extends StatefulWidget {
  const BuildTotalStats({super.key, required this.globalStats});
  final Future<List<Map<String, dynamic>>> globalStats;

  @override
  State<BuildTotalStats> createState() => _BuildTotalStatsState();
}

class _BuildTotalStatsState extends State<BuildTotalStats> {
  @override
  Widget build(BuildContext context) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: widget.globalStats,
      builder: (
        BuildContext context,
        AsyncSnapshot<List<Map<String, dynamic>>> snapshot,
      ) {
        if (snapshot.hasData) {
          List<Map<String, dynamic>> globalStats = snapshot.data!;
...
class TotalStats extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final String currHsk =
        globalStats["current_hsk"] == "2"
            ? "1-2"
            : globalStats["current_hsk"].toString();
    return Container(
            ...
            children: [
              Column(
                children: [
                  Text("hsk $currHsk"),
                  Text(
                    "${globalStats["percent_current_hsk_completed"].toString().substring(0, 3)}%",
                  ),
                ],
              ),
              Column(
        ...
        child: Column(
          children: [
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceAround,
              children: [
                Column(children: [const Text("New"), Text(newWords)]),
                Column(children: [const Text("Reviewed"), Text(reviewWords)]),
                Column(
                  children: [const Text("Correct"), Text("$successRate%")],
                ),
                Column(
                  children: [
                    ...
                    TextButton(
                      ...
                      onPressed: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:
                                (context) => WordView(wordId: hskList["id"]),
                          ),
                        );
                      },
                      child: Text(
